...
import { ProductType } from "@/features/shopping-cart/type";
...
const ProductItem = ({ product }: ProductItemProps) => {
  ...
  const cartItem = useCartStore((state) =>
    state.cart.find((prod) => prod.product.id === product.id),
  );
  return (
        ...
        {cartItem ? (
          <div className="flex items-center gap-3 text-xl font-medium">
            ...
            <Button
              ...
              className="px-4 text-2xl bg-emerald-300 text-emerald-700"
            >
              +
            </Button>
          </div>
        ) : (
          <Button onClick={() => addToCart(product)} className="bg-emerald-500">
            Add to Cart
          </Button>
        )}
      </CardContent>
